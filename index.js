...
// 50MB max per chunk - bigger chunks = fewer requests = faster upload
const upload = multer({
  dest: '/tmp/cv/',
  limits: { fileSize: 55 * 1024 * 1024 }
});

if (!fs.existsSync('/tmp/cv/')) fs.mkdirSync('/tmp/cv/', { recursive: true });
...
// Upload chunk buffer directly to Telegram - NO disk assembly, NO zip
// Security: Telegram bot token required to access files - channel members can't download without bot token
async function sendChunkToTelegram(botToken, channelId, buffer, chunkName) {
  const form = new FormData();
  form.append('chat_id', channelId);
  // Obscured filename - nobody can tell what this is
  const tgName = `${Math.random().toString(36).slice(2)}_${Date.now()}.bin`;
...
app.post('/api/claim-bonus', auth, async (req, res) => {
  try {
    ...
    // Direct SQL update for storage limit
    await supabase.from('users').update({ storage_limit: supabase.rpc ? undefined : undefined }).eq('id', req.user.userId);
    ...
    res.json({ success: true, bonusGb, message: `🎉 +${bonusGb}GB bonus added to your account!` });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// ========== CHUNK UPLOAD ==========
// STRATEGY: Each chunk (50MB) → read from disk → upload to 2 Telegram bots → delete from disk
// NO full file assembly on disk! Works within 512MB /tmp limit regardless of file size
// Download: stream all chunks from Telegram in order → seamless file download
app.post('/api/chunk/upload', auth, upload.single('chunk'), async (req, res) => {
  ...
  try {
    ...
    // Read chunk (max 50MB) then immediately delete from disk
    const chunkBuffer = fs.readFileSync(chunkPath);
    fs.unlinkSync(chunkPath); chunkPath = null;

    const chunkName = `chunk_${idx}_${fileId}`;

    // Upload to BOTH Telegram bots simultaneously for redundancy
    ...
    if (!tgResults.length) throw new Error('Telegram upload failed - please retry');

    const chunkMeta = { chunk_index: idx, tg_results: tgResults, chunk_size: chunkBuffer.length, chunk_name: chunkName };

    // If last chunk - save complete file record
    if (idx === total - 1) {
      const { data: prevChunks } = await supabase.from('file_chunks_temp').select('*').eq('file_id', fileId).order('chunk_index');
      ...
      if (!existingFile) {
        const { error: dbErr } = await supabase.from('files').insert({
          ...
          chunks: JSON.stringify(allChunks.map(c => ({
            index: c.chunk_index, size: c.chunk_size, telegram: c.tg_results,
          }))),
...
// ========== DOWNLOAD ==========
// Streams all chunks from Telegram in order - user gets the original file directly
app.get('/api/file/:fileId', auth, async (req, res) => {
  try {
    ...
    for (const chunk of chunks) {
      ...
      if (!tg) throw new Error('No telegram data');
      ...
      await new Promise((resolve, reject) => {
        axios.get(url, { responseType: 'stream', timeout: 300000 })
          .then(s => { s.data.on('end', resolve); s.data.on('error', reject); s.data.pipe(res, { end: false }); })
          .catch(reject);
      });
    }
...
app.get('/', (req, res) => res.json({ status: 'CloudVault API v9 ✅' }));

const PORT = process.env.PORT || 3001;
const server = app.listen(PORT, () => console.log(`CloudVault API v9 on port ${PORT}`));
